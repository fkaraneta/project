...
<body>
	...
	<?php		
		...
		?>		
				...
				<td  colspan="5"><?php echo "₱ ".$item["prod_price"]; ?> x<?php echo $item["quantity"]?> <a href="Checkout.php?username=<?php  echo $row['username']; ?>&Custmerid=<?php echo $row['Custmerid']; ?>&edit=<?php echo $row['Custmerid']; ?>&action=remove&code=<?php echo $item["code"]; ?>" class="btnRemoveAction">Delete</a></td></tr>
				...
				<tr>
					<td colspan="3" class="viewmycart"><a href="viewcart.php?username=<?php  echo $row['username']; ?>&Custmerid=<?php echo $row['Custmerid']; ?>&edit=<?php echo $row['Custmerid']; ?>" class=""><p class="color-text">View My Cart</p></a></td>
				</tr>
 		 <?php
			} else {
			?>
		...
		<center>
			<form method="post" action="Home.php?username=<?php  echo $row['username']; ?>&Custmerid=<?php echo $row['Custmerid']; ?>&edit=<?php echo $row['Custmerid']; ?>&action=remove&code=<?php echo $item["code"]; ?>">
		<div class="checkArrange">
		<table>
			<tr>
				<th>Delivery Address</th>
			</tr>
			<tr>
				<th><textarea class="AddressText" name="address" required placeholder="Your Address here...."></textarea></th>
...
<?php		
    foreach ($_SESSION["cart_item"] as $item){
		...
		?>
				...
				<input type="hidden" name="images" value="<?php echo $item['image'];?>">
				<input type="hidden" name="dname" value="<?php echo $item["dname"]; ?>">
				<td><?php echo $item["code"]; ?></td><input type="hidden" name="code" value="<?php echo $item["code"]; ?>">
...
<td align="right"><?php echo $total_quantity; ?></td><input type="hidden" name="qty" value="<?php echo $total_quantity; ?>">
...
<td></td>
		...
		<br>
		<button class="btncheck" type="submit" name="buy">Place Order</button></a>

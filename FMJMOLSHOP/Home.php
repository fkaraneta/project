...
<body>
	...
	<?php		
		...
		?>		
				...
				<td  colspan="5"><?php echo "₱ ".number_format($item["prod_price"],2); ?> x<?php echo $item["quantity"]?> <a href="Home.php?username=<?php  echo $row['username']; ?>&Custmerid=<?php echo $row['Custmerid']; ?>&edit=<?php echo $row['Custmerid']; ?>&action=remove&code=<?php echo $item["code"]; ?>" >Delete</a></td></tr>
				<td olspan="5"></td>
				<?php
				...
				<tr>
					<td colspan="3" class="viewmycart"><a href="Payment.php?username=<?php  echo $row['username']; ?>&Custmerid=<?php echo $row['Custmerid']; ?>&edit=<?php echo $row['Custmerid']; ?>"><p class="color-text">View My Cart</p></a></td>
				</tr>
 		 <?php
			} else {
			?>
	...
	?>	
			...
			<form method="post" action="Home.php?username=<?php  echo $row['username']; ?>&Custmerid=<?php echo $row['Custmerid']; ?>&edit=<?php echo $row['Custmerid']; ?>&action=add&code=<?php echo $value["code"]; ?>">
			...
			<div class ="main">
					...
					<div><input type="hidden" class="product-quantity" name="quantity" value="1" size="2" />
						<button value="Add to Cart" class="btn btn4" />Add to Cart</button></div>	
			</div>
...
$db = mysqli_connect('localhost','root','','fmjm') or ("Could not connect database");
    ...
    $sql1 = "INSERT INTO `transaction`(`Customername`, `img_product`, `ProductName`,`code`,`QTY`,`Price`,`Delivery_Address`, `Date_Purchased`) VALUES ('$name','$image','$product','$code','$qty','$price','$address','$date')";
      header('location: Home.php ');
    $quers=mysqli_query($db, $sql1);
    // execute query
   if($quers ){
